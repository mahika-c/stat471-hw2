...
pacman::p_load(ISLR, tidyverse, data.table, magrittr, ggbiplot, irlba, car) # add the packages needed!!!
set.seed(0)
```
...
str(auto)
summary(auto)
auto2 <- copy(auto) # save a copy for future computations
...
mpg (miles per gallon) - int: fuel efficiency metric cylinders - factor: number of cylinders in the car, 4-8 displacement - int: engine displacement in cubic inches horsepower - int: the rate at which work is done by the car (power metric) weight - int: the weight of the car in lbs acceleration - num: time to accelerate from 0 to 60 mph in seconds year - num: the model year of the car, modulo 100 origin - factor: either 1 American, 2 European, or 3 Japanese
...
```{r echo=FALSE}
summary(auto)
auto <- subset(auto, !(cylinders == 3))
auto2 <- subset(auto2, !(cylinders == 3))
summary(auto)
```
...
```{r echo=FALSE}
# Scatterplot of MPG versus acceleration
ggplot(auto, aes(x = acceleration, y = mpg)) +
  geom_point() +  
  labs(x = "Acceleration", y = "Miles per Gallon", title = "MPG vs Acceleration") +  
  theme_minimal() 
```
...
```{r echo=FALSE}
# run regression with MPG vs year
mpg_lm1 <- lm(mpg ~ year, data = auto) 
summary(mpg_lm1)
...
```{r echo=FALSE}
# run regression with MPG vs year and horsepower
mpg_lm2 <- lm(mpg ~ year + horsepower, data = auto) 
summary(mpg_lm2)
```
...
```{r echo=FALSE}
# run regression with MPG vs cylinders as numeric
mpg_lm3 <- lm(mpg ~ cylinders, data = auto2) 
summary(mpg_lm3)

# plot(mpg_lm3, 1)
ggplot(auto2, aes(x = cylinders, y = mpg)) + 
  geom_point() +
  geom_smooth(method="lm",se=F) + 
  ggtitle("MPG vs Cylinders as numeric variable")
geom_hline(aes(yintercept = mean(mpg)), color = "red") 
```
...
Here, we create a boxplot to show the spread of mgp given the number of cylinders.
...
ggplot(auto) + geom_boxplot(aes(x = cylinders, y = mpg))
```

```{r echo=FALSE}
# run regression with MPG vs cylinders as categorical
mpg_lm4 <- lm(mpg ~ cylinders, data = auto) 
summary(mpg_lm4)
...
ggplot(auto, aes(x = cylinders, y = mpg)) + 
  geom_point() +
  geom_smooth(method="lm",se=F) + 
  ggtitle("MPG vs Cylinders as categorical variable")
geom_hline(aes(yintercept = mean(mpg)), color = "red") 
```
...
We ran an anova analysis for the two different models, where fit0 is represented by mpg_lm3, and fit1 is represented by mpg_lm4. We see that our p-value is very small, 3.7e-06. Thus, at the 0.01 level, we can reject the null hypothesis. We thus think that we should treat cylinders as a categorical variable.

```{r echo=FALSE}
anova(mpg_lm3, mpg_lm4)  
```
...
```{r echo=FALSE}
# we see a very low R-squared, so we don't include acceleration in our final model
fit.acceleration <- lm(mpg ~ acceleration, data = auto)
summary(fit.acceleration)
```
...
```{r echo=FALSE}
# run regression with MPG vs other variables
mpg_lm5 <- lm(mpg ~ year * origin + cylinders + displacement + weight + horsepower, data = auto) 
summary(mpg_lm5)
```
...
```{r results=TRUE}
newcar <- data.frame(cylinders = as.factor(8),
                     displacement = 350, 
                     weight = 4000,
                     origin = as.factor(1),
                     horsepower = 260,
...
```{r results=TRUE}
predict(mpg_lm5, newcar, interval = "confidence", se.fit = TRUE) 
```

Based on the prediction above with our final model, we can see that the prediction is 16.6 mpg +/- 2 \* 1.31 for a 95% CI, which gives us lower and upper bounds of 14 and 19.2 mpg.
