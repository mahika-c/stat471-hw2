...
num_gene <- ncol(brca)
svd_var <- svd_ret$d^2/(nrow(brca_sub_unscaled_centered)-1)
pve_apx <- svd_var/num_gene
plot(pve_apx, type="b", pch = 19, frame = FALSE)
```
...
pc_score_scaled <- brca_sub_scaled_centered %*% svd_ret$v[, 1:5] #from PCA formula
pc_score_scaled <- (svd_ret$u[, 1:5])*(svd_ret$d[1:5])  # from svd 
...
pc_score_unscaled <- brca_sub_unscaled_centered %*% svd_ret$v[, 1:5] #from PCA formula
pc_score_unscaled <- (svd_ret$u[, 1:5])*(svd_ret$d[1:5])  # from svd 
